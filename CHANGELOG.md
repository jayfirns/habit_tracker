...
## Unreleased - Timer Persistence Bug Fix

### Fixed
- **Bug**: Frontend timer functions (`toggleHabitTimer`, `stopHabitTimer` in `app.js`) now properly handle API errors instead of using fire-and-forget calls. If backend calls fail, users see "Timer sync failed" status message.

### Added
- **Tests**: Added `timer-integration.test.mjs` with 6 tests verifying timer API integration and error handling.

### Changed
- **Docs**: Updated `LOGIC_RULES.md` to reflect the fix - timer functions now include proper error handling.
- **Docs**: Updated `EnergyMixPanel.md` to note timer sync includes error handling.
